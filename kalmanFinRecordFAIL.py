...
frame_capture_interval = 0.1  # Time interval (in seconds) between each frame capture
last_capture_time = 0

# Data collection lists
positional_error = []  # Store positional errors (true_state - kf.x)
positional_uncertainty = []  # Store positional uncertainties (diagonal of kf.P)
...
initial_covariance_value = 20000
Q = np.eye(8) * .001 #increases how fast the uncertainty expands
Q[3][3] = 0.0001
Q[4][4] = 0.0001
Q[6][6] = 0.00001
...
video_duration = 150  # Duration of the video in seconds
turn_time = 0
total_turn_time = 0
while time <= video_duration:
    ...
    Q_rotate = rotate_covariance_matrix(orientation_rad, 10, .1) 
    # # print(Q_rotate, "1")
    Q[0:2, 0:2] = Q_rotate 
    ...
    error = np.linalg.norm(true_state[:2] - kf.x[:2])
    uncertainty = np.trace(kf.P[:2, :2])
    positional_error.append(error)
    positional_uncertainty.append(uncertainty)

    # Capture frames for GIF
    #if time - last_capture_time >= frame_capture_interval:
...
# imageio.mimsave('pygame_simulation.gif', frames, fps=50)  # Adjust fps as needed

# # Plotting the graphs
# plt.figure(figsize=(10, 5))
# plt.subplot(1, 2, 1)
# plt.plot(positional_error)
# plt.title('Positional Error over Time')
# plt.xlabel('Time Steps')
# plt.ylabel('Error')

# plt.subplot(1, 2, 2)
# plt.plot(positional_uncertainty)
# plt.title('Positional Uncertainty over Time')
# plt.xlabel('Time Steps')
