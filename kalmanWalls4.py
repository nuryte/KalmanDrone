...
initial_covariance_value = 200

Q = np.eye(9) * .0001 #increases how fast the uncertainty expands
...
while True:
    ...
    true_state[0] += true_state[3] * dt * math.cos(orientation_rad)  + 2 * dt # Update x position
    ...
    if distance_to_wall < 100:# and turn_time <= 0:#position_measurement_available(true_state):
        ...
        if wall_collide is not None:
            if wall_collide.rect.width > wall_collide.rect.height:
                ...
                oriented_dist = wall_collide.rect.y - np.cos(orientation_rad) * distance_to_wall + np.random.normal(0, 10)
                
            else:
                H_tof = np.array([[1, 0, 0, 0, 0, 0, 0, 0, 0]])  # Measure x_position
                # if wall_collide.rect.y < wall_collide.rect.height:
                oriented_dist = wall_collide.rect.x - np.sin(orientation_rad) * distance_to_wall + np.random.normal(0, 10)
        # else:
