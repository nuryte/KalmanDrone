...
def move_robot(true_state, control_inputs, walls):
    ...
    true_state[0] += true_state[3] * dt * math.cos(orientation_rad + angle_noise) + (drift_x + wind_x* (np.random.normal(1, .2)))* dt # Update x position
    true_state[1] += true_state[3] * dt * math.sin(orientation_rad + angle_noise) + (drift_y + wind_y* (np.random.normal(1, .2)) )* dt # Update y position
...
def get_position_measurement(true_state, stored_state):
    # Simulate Measurement with Some Noise for x and y position
    measured_position = np.array([[0.0], [0.0], [0.0], [0.0]])
    position_noise = np.random.normal(0, 20, size=(2, 1))  # Noise with standard deviation of 10
    measured_position[0:2] = true_state[:2] + position_noise
    measured_position[2:4] = true_state[:2] - stored_state
...
stored_state = np.array([[400.0], [300.0]])
...
H_position = np.array([[1, 0, 0, 0, 0, 0,0,0],  # Measure x_position
                    [0, 1, 0, 0, 0, 0,0,0], # Measure y_position
                    [0, 0, 0, 1, 0, 0,0,0], # Measure x_wind
                    [0, 0, 0, 0, 1, 0,0,0]]) # Measure y_wind
...
while True:
    ...
    position_measurement = get_position_measurement(true_state, stored_state)
    R_postemp = np.eye(4) * 1000#rotate_covariance_matrix(orientation_rad, sigma_radial, sigma_perpendicular)
    R_postemp[2:4] *= 10
    kf.update(position_measurement, H_position, R_postemp)
    stored_state[0,0] = kf.x[0,0]
    stored_state[1,0] = kf.x[1,0]
    ...
    if angle_measurement_available(true_state) and angular_velocity == 0:
