...
initial_covariance_value = 200000
Q = np.eye(8) * 10 #increases how fast the uncertainty expands
Q[6][6] = 0
Q[7][7] = 0
...
while True:
    ...
    Q_rotate = rotate_covariance_matrix(orientation_rad, 1, .1) 
    # # print(Q_rotate, "1")
    Q[0:2, 0:2] = Q_rotate 
    ...
    if distance_to_wall < detection_distance:# and turn_time <= 0:#position_measurement_available(true_state):
        ...
        if wall_collide is not None:
            if wall_collide.rect.width > wall_collide.rect.height:
                ...
                oriented_dist = wall_collide.rect.y - np.sin(orientation_rad) *  np.random.normal(distance_to_wall, distance_to_wall * 0)
                ...
                oriented_dist = wall_collide.rect.x - np.cos(orientation_rad) *  np.random.normal(distance_to_wall, distance_to_wall * 0)
