...
while True:
    ...
    if distance_to_wall < detection_distance:# and turn_time <= 0:#position_measurement_available(true_state):
        ...
        if wall_collide is not None:
            if wall_collide.rect.width > wall_collide.rect.height:
                ...
                oriented_dist = wall_collide.rect.y - np.sin(orientation_rad) *  np.random.normal(distance_to_wall, distance_to_wall * .1)
                ...
                oriented_dist = wall_collide.rect.x - np.cos(orientation_rad) *  np.random.normal(distance_to_wall, distance_to_wall * .1)
    ...
    control_text = f"correction: x={( kf.x[6,0]/ dt):.2f}, y={ (kf.x[7,0]/dt):.2f}, drift: x={( drift_x):.2f}, y={ (drift_y):.2f}, wind: x={( wind_x):.2f}, y={ (wind_y):.2f}  "
